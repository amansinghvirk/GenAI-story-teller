...
class FormatStory:
    ...
    def add_part(self, image_path, story, section, back_color, font_color):
        ...
        if section % 2 == 0:
            ...
            <div style="background-color: {back_color}; color: {font_color}; margin: auto; box-shadow: 10px 10px 20px 10px ; border-radius: 25px;">
    ...
    def compile_story(self):
        self.story = f'''
                ...
                <div style="max-width: 1000px; padding: 10px;   margin: auto;
                    padding: 10px; box-shadow: 10px 10px 20px 10px;">
                    {self.title}
                    {self.story_intro}
                    {self.story_parts}
                </div>
